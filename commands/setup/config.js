    ...
    async execute(interaction, client){
        ...
        switch(interaction.options.getSubcommand()){
            case 'salon':
                ...
                if(!channel.permissionsFor(interaction.guild.fetchMe(), checkAdmin = true).has(PermissionsBitField.Flags.SendMessages)) return interaction.editReply({ content: "Je n'ai pas la permission d'écrire dans le salon indiqué !" });
                if(!channel.permissionsFor(interaction.guild.fetchMe(), checkAdmin = true).has(PermissionsBitField.Flags.CreateInstantInvite)) return interaction.editReply({ content: "Je n'ai pas la permission de créer une invitation dans le salon indiqué !" });
